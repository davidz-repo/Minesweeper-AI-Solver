...
class World():
	...
	def __init__(self, filename=None, aiType="myai", verbose=False, debug=False):
		...
		if aiType == "manual":
			...
			self.ai = RandomAI(self.__rowDimension, self.__colDimension, self.__totalMines, firstMoveCoords[0], firstMoveCoords[1])
		elif aiType == "myai":
			self.ai = MyAI(self.__rowDimension, self.__colDimension, self.__totalMines, firstMoveCoords[0], firstMoveCoords[1])
	...
	def run(self) -> int:
		""" Engine of the game """
		while (True):
			if type(self.ai) == ManualAI or self.__debug:
				self.__printWorld()
			if self.__movesMade > self.__movesLimit:
				break;

			try:
				action = self.ai.getAction(self.perceptNumber)
			...
			if self.__debug and type(self.ai) != ManualAI:
	...
	def __uncoverTile(self, c: int, r: int) -> None:
		...
		self.perceptNumber = self.__board[c][r].number
