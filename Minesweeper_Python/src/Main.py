...
def main():
    ...
    parser = argparse.ArgumentParser(
                    description="", prog="Main.py", usage="%(prog)s [options]",
                    epilog="Note: [options] can be in any order")

    # Parse options
    # Help is default
    parser.add_argument("-f", "-F",
                        help="file or directory name", nargs='*')
                        # File path

    parser.add_argument("-m", "-M",
                        help="enable ManualAI mode", action="store_true")
                        # ManualAI

    parser.add_argument("-r", "-R",
                        help="enable RandomAI mode", action="store_true")
                        # RandomAI
    ...
    if inputFile:
        ...
        if (os.path.isdir(inputFile)):
            ...
            for dirpath, _, filenames in directory:
                for filename in filenames:
                    ...
                    score = world.run()
                    gamenumber += 1
                    print(" Finished:", gamenumber, "A:", scoreBeg, "B:", scoreInt, "C:", scoreExp)
                    if score == 1:
                        scoreBeg += 1
                    elif score == 2:
                        scoreInt += 1
            ...
            print("Beginner: {} \tIntermediate: {} \tExpert: {}"
                    .format(scoreBeg, scoreInt, scoreExp))
        ...
        # If inputFile is a world file
            ...
            world = World(filename=inputFile,
                          aiType=aiType,
                          verbose=verbose,
                          debug=debug)
            score = world.run()
